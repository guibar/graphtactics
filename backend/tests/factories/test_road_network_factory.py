...
class TestIsValidBbox:
    ...
    def temp_bbox_file(self) -> Generator[str, None, None]:
    ...
    def test_valid_bbox_exists(self, temp_bbox_file: str):
    ...
    def test_invalid_box_values(self, temp_bbox_file: str):
...
class TestCreate:
    ...
    def test_create_from_cache_when_files_exist(self, monkeypatch: MonkeyPatch):
        ...
        def mock_isfile(path: str | Path) -> bool:
            return True
        ...
        # Track calls
        instantiate_called: list[bool] = []
        download_called: list[bool] = []
        create_files_called: list[bool] = []

        # Mock the instantiate_from_files method
        def mock_instantiate() -> RoadNetwork:
        ...
        def mock_download(name: str) -> bool:
        ...
        def mock_create(boundary: Any) -> None:
    ...
    def test_download_and_instantiate_from_github(self, tmp_path: Path):
        ...
        factory.cache_dir = str(tmp_path)
    ...
    def test_create_network_from_scratch(self, tmp_path: Path):
        """Test creating files from boundary (integration test)."""
        # Create factory with default GitHub settings
        # Use tmp_path as cache_dir to avoid polluting actual cache

        factory = RoadNetworkFactory(cache_dir=str(tmp_path))
        # name: str = "st_quentin"
